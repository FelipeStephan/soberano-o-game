...
//
// Agora 75–170s, e no MÁXIMO um por ciclo: o flash volta a ser um susto.
const ATRASO_MIN = 75000;
const ATRASO_VAR = 95000;
let flashNesteCiclo = 0;
let turnoDoFlash = -1;

export function agendarFlash(jogo, aoResolver) {
  cancelarFlash();
  // Um por ciclo. Dois seguidos no mesmo turno é o que fazia parecer metralhadora.
  if (turnoDoFlash !== jogo.turno) { turnoDoFlash = jogo.turno; flashNesteCiclo = 0; }
  if (flashNesteCiclo >= 1) return;
  ...
  timerAgendado = setTimeout(() => {
    ...
    if (flash) { flashNesteCiclo += 1; mostrarFlash(flash, jogo, aoResolver); }
  }, atraso);
}
