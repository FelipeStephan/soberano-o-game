...
export function abrirMercado(jogo, { onFim } = {}) {
  ...
  function fichaHTML(it) {
    ...
    return `<div class="mf">
      ...
      ${av.pode ? `
        <div class="mf-qtd">
          <label>Quantidade <b id="mf-q">${qtd.toLocaleString('pt-BR')}</b></label>
        ...
        ${av.nacional
          ...
          : `<div class="mf-nota">${ico('info', 12)} O dinheiro fica em custódia até ${esc(PAISES[it.origem]?.nome || it.origem)} responder, no fim do ciclo. Se negarem, ele volta — mas a negativa vira notícia.</div>`}
        ${semPA ? `<div class="mf-nota ruim">${ico('ban', 12)} Sem pontos de ação neste ciclo.</div>` : ''}
      ` : `
        <div class="mf-bloq">
          ${ico('lock', 20)}
  ...
  // O slider atualiza só os números — redesenhar a ficha inteira mataria o arrasto.
  function ligarFicha() {
    ...
    sl?.addEventListener('input', () => {
      ...
      const total = round4(preco * qtd);
      modal.querySelector('#mf-q').textContent = qtd.toLocaleString('pt-BR');
    ...
    modal.querySelector('#mf-go')?.addEventListener('click', () => {
  ...
  function corpoPedidos() {
    ...
    return `<div class="mk-pedidos">
      ...
      <div class="mkp-topo">${ico('info', 13)} <span>Respondidos no fechamento do ciclo, usando a relação <b>daquele momento</b>. Ataque um aliado agora e o pedido dele morre junto.</span></div>
      ${ps.map((p) => {
      ...
      ${hist.map((h) => {
        ...
        return `<div class="mkp hist ${sCls}">
          ...
          <div class="mkp-txt">
            ...
            <small>${esc(PAISES[h.origem]?.nome || h.origem)} · ${dinheiro(h.total)} · ciclo ${h.turnoResposta}</small>
