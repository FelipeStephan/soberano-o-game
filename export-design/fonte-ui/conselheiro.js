...
export function abrirConselheiro(jogo, { onAplicar, onFim } = {}) {
  ...
  const cabecalho = (selo) => `
    <div class="cs-cab">
      ...
      <div class="cs-tit">
        <h2>CONSELHEIRO</h2>
        <div class="cs-sub">Briefing de gabinete · ciclo ${jogo.turno ?? '?'}</div>
      </div>
      ${selo}
  ...
  gerarConselho(jogo, {}).then((conselho) => {
    ...
    pintar(`<div class="cs-erro">${ico('triangle-alert', 16)} O gabinete não conseguiu fechar a análise. Confie no seu faro neste ciclo.</div>`);
  });
    ...
    modal.querySelectorAll('.cs-aplicar').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (res.ok) {
          ...
          btn.outerHTML = `<div class="cs-ok">${ico('check', 14)} Ação enfileirada para este ciclo</div>`;
          onAplicar?.(acao);
        } else {
          btn.disabled = true;
