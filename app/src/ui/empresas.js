  ...
  function render() {
    ...
    modal.innerHTML = `<div class="emp-painel">
      <div class="emp-cab">
        ...
        <div class="emp-lucro"><span>Lucro / turno</span><b class="${lucro >= 0 ? 'bom' : 'ruim'}">${dinheiro(lucro)}</b></div>
  ...
  function detalhe() {
    ...
    alvo.innerHTML = `
      ...
      <div class="ed-stats">
        ...
        <div class="ed-s"><span>Te rende</span><b class="bom">${dinheiro(lucroDela)}<small>/turno</small></b></div>
      </div>
      ...
      ${e.bonus && e.participacao >= 20 ? `<div class="ed-bonus">${ico('sparkles', 12)} Enquanto você tem ≥20%: ${Object.entries(e.bonus).map(([k, v]) => `<span class="mud bom">${k} +${(v * e.participacao / 100).toFixed(1)}/turno</span>`).join(' ')}</div>` : ''}
